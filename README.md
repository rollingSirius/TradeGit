...
TradeGit 不绑定任何券商。内置解析器只是为了少做字段整理；只要能导出 CSV / JSON，
就可以通过通用导入格式进入同一套 journal schema。
...
具体导出步骤、字段映射和已知的坑见 [`reference/brokers.md`](reference/brokers.md)。
...
所有指标都由记录里的成交、费用、现金事件和风险字段计算；导入文件只提供原始事实，
不会把券商自己的平仓配对结果当成计算依据。
...
测试覆盖字段归一、P&L 计算、导入解析器、存储去重与索引刷新、凭证不外泄的安全保证，以及一整套用本地 bare 仓库当
